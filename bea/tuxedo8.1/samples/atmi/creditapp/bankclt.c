...
static int	CBalance(void);
static int	CPurchase(void);
...
static int	COpenAccount(void);
static int	CCloseAccount(void);
...
char *menu = "\n\n\
           ...
           Make a selection from the following choices\n\
               Credit Card Services\n\
                    ...
                    4. Close Credit Account\n\
               Bank Services\n\
                    5. Deposit to Bank Account\n\
                    6. Withdrawal from Bank Account\n\
                    7. Bank Balance Inquiry\n\
                    8. Transfer Between Bank Accounts\n\
                    9. Open Bank Account\n\
...
\n\
                   Enter Selection: ";
char *menu2 = "\n\n\
                      Welcome to the Tuxedo Credit Card Authorization Center\n\
           Make a selection from the following choices\n\
                    1. Authorize Payment\n\
                    2. Balance Inquiry\n\
                    3. Open Credit Account\n\
...
main(int argc, char **argv)
{
	...
	if (argc > 1 && strcmp(argv[1],"CREDITONLY") == 0)
		creditapp = 1;
	...
	do {
		...
		if (creditapp)
			(void)fprintf(stdout, "%s", menu2);
		else
			(void)fprintf(stdout, "%s", menu);
		(void)fflush(stdout);
		...
		if (creditapp && option[0] > '4')
			continue;
		switch(option[0]) {
		case '1':		/* Authorize Purchase */
			if (option[1] == '\0') {
				if(CPurchase() == -1)
					done = 1;
			}
			...
			break;
		case '2':		/* Balance Inquire */
			if(CBalance() == -1)
				done = 1;
			break;
		case '3':		/* Open Credit Account */
			if(COpenAccount() == -1)
				done = 1;
			break;
		case '4':		/* Close Credit Account */
			if(CCloseAccount() == -1)
				done = 1;
			break;
		case '5':		/* Deposit */
...
CPurchase(void)
{
	...
	Finit(fbfr, (FLDLEN)Fsizeof(fbfr));
	/* Add the Account number and the amount to the buffer */
	(void) Fchg(fbfr, C_ACCOUNT_ID, 0, (char *)&acc_num, 0);
	(void) Fchg(fbfr, SAMOUNT, 0, amount, 0);

	/* send a request to the WITHDRAWA service and get the reply */
	if (do_tpcall("WITHDRAWALA", 1) == 0) {
		/* if Success the result is in the FML buffer */
		(void)fprintf(stdout,
...
COpenAccount(void)
{
	...
	(void) Fchg(fbfr, PHONE, 0, phone, 0);
again:
	(void)fprintf(stdout, "Account Type M - Mastercard V - Visa: ");
	(void)fflush(stdout);
	if(get_val(acct_type, sizeof(acct_type),
			"Please Enter Account Type: ") == -1)
		return(0);
	if(strcmp(acct_type, "M") != 0 && strcmp(acct_type, "V") != 0)
		goto again;
	(void) Fchg(fbfr, C_ACCT_TYPE, 0, acct_type, 0);
...
"6-Atlanta", "7-Maimi", "8-Philadelphia", "9-New York", "10-Boston");
	...
	/* send a request to the OPENCA service and get the reply */
	if (do_tpcall("OPENCA", 1) == 0) {
		/* if Success the result is in the FML buffer */
		Fget(fbfr, C_ACCOUNT_ID, 0, (char *)&acc_num, 0);
		(void)fprintf(stdout, "\nNew Account Opened\n");
		(void)fprintf(stdout, "\nAccount Number: %ld\n", acc_num);
		f = 0.0;
		(void) Fget(fbfr, C_BAMOUNT, 0, (char *)&f, 0);
		(void)fprintf(stdout, "\nInitial Balance: %.2f\n", f);
		(void) Fget(fbfr, C_CAMOUNT, 0, (char *)&f, 0);
		(void)fprintf(stdout, "\nCredit Line: %.2f\n", f);
	}
	return(0);
}
...
CCloseAccount(void)
{
	...
	/* send a request to the CLOSECA service and get the reply */
	if (do_tpcall("CLOSECA", 1) == 0) {
		(void)fprintf(stdout, "\nAccount %ld Closed\n", acc_num);
	}
	return(0);
}
 ...
 */
static int
do_tpcall(char *service, int notran)
{
...
get_account(char *message)
{
	...
	do {
		...
		for (i=0; account[i] != 0 && i <= 9; i++) {
			if (!isdigit(account[i]))
				break;
			acc_num = acc_num * 10 + (account[i] - '0');
		}
		if (i < 5 || i > 9 || account[i] != '\0') {
			(void)fprintf(stderr,
			"ERROR: Account number must be 5 to 9 digits\n");
			acc_num = 0;
			continue;
		}
		break;
