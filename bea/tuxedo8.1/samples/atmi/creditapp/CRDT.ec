...
*/
#ident	"@(#) samples/atmi/creditapp/CRDT.ec	$Revision: 1.5 $"
#ifndef NOWHAT
static	char	sccsid[] = "@(#) samples/atmi/creditapp/CRDT.ec	$Revision: 1.5 $";
#endif
...
#endif
{
	...
	if (Fget(transf, CAMOUNT, 0, amts, &len) < 0 
		|| strcmp(amts,"") == 0) {
		(void)Fchg(transf, STATLIN, 0, "Invalid amount",(FLDLEN)0);
		tpreturn(TPFAIL, 0, transb->data, 0L, 0);
	}
	(void)sscanf(amts,"%f",&c_bal);

	if (Fget(transf, BAMOUNT, 0, amts2, &len) < 0 
		|| strcmp(amts2,"") == 0) {
		(void)Fchg(transf, STATLIN, 0, "Invalid amount",(FLDLEN)0);
		tpreturn(TPFAIL, 0, transb->data, 0L, 0);
	}
	(void)sscanf(amts2,"%f",&b_bal);

	/* must have valid amount to deposit */
	if (c_bal <= 0.0) {
		(void)Fchg(transf, STATLIN, 0,
			"Initial credit must be greater than $0.00",(FLDLEN)0);
		tpreturn(TPFAIL, 0, transb->data, 0L, 0);
	}
...
userlog ("BEFORE SQL4 ");
	...
	(void)Fchg(transf, BAMOUNT, 0, (char *)&b_bal, (FLDLEN)0);
	(void)Fchg(transf, CAMOUNT, 0, (char *)&c_bal, (FLDLEN)0);
	tpfree((char *)reqfb);
...
#endif
{
	...
	/* prepare buffer for successful return */
	(void)Fchg(transf, SBALANCE, 0, Fvals(reqfb,C_BAMOUNT,0), (FLDLEN)0);
