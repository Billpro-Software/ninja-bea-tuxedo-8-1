...
*/
/* #ident	"@(#) samples/atmi/ws/BANKAPPW.C	$Revision: 1.5 $" */
#ifndef __STDC__
#define __STDC__ 0
#endif
...
#else
	if ((tpinfop = (TPINIT FAR *)tpalloc("TPINIT", NULL,
		sizeof(struct tpinfo_t))) == (TPINIT *)NULL) {
		userlog("unable to get init buf\n");
		return(0);
	}
	lstrcpy(tpinfop->usrname, "Smith");
	lstrcpy(tpinfop->cltname, "bankapp");
	lstrcpy(tpinfop->passwd, "mypasswd");
...
WndProc(HWND hwnd, WORD message, WORD wParam, LONG lParam)
{
	...
	switch (wParam) {
	case IDM_BALANCE:
		DialogBox (hInst, "BALANCE", hwnd, lpfnBalanceDlg);
		return(0);
	case IDM_DEPOSIT:
		DialogBox (hInst, "DEPOSIT", hwnd, lpfnDepositDlg);
		return(0);
	case IDM_TRANSFER:
		DialogBox (hInst, "TRANSFER", hwnd, lpfnTransferDlg);
		return(0);
	case IDM_WITHDRAWAL:
		DialogBox (hInst, "WITHDRAW", hwnd, lpfnWithdrawDlg);
		return(0);
	case IDM_OPEN:
		DialogBox (hInst, "OPEN", hwnd, lpfnOpenDlg);
		return(0);
	case IDM_CLOSE:
		DialogBox (hInst, "CLOSE", hwnd, lpfnCloseDlg);
		return(0);
	}
	break;
