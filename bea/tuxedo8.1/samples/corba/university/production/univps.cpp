...
static void register_fact()
{
    ...
    // registrar factories to co-exist (for scaling)
    ostrstream os;
...
BillingP::TellerFactory_var s_v_tell_fact_ref;
...
static void init_teller_factory()
{
    ...
    s_v_tell_fact_ref =
        BillingP::TellerFactory::_narrow(v_fact_oref.in());
}
...
CORBA::Boolean Server::initialize(int argc, char* argv[])
{
    ...
    try {
        ...
        for (int i = 1; i < argc; i++) TRACE(argv[0] << ' ' << argv[i]);
...
Tobj_Servant Server::create_servant(const char* intf_repos_id)
{
    ...
    try {
        ...
        if (!strcmp(intf_repos_id, reg_repos_id)) {
            return new Registrar_i(*s_p_db, s_v_tell_fact_ref.in());
        }
