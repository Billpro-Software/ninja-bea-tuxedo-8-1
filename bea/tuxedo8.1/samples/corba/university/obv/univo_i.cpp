...
//--------------------------------------------------------------------
// utility to retrieve a course from the database and
// package it up to send back thru its corresponding IDL type

static void get_crs(
    Database&                   db,
    UniversityO::CourseNumber   crs_num,
    Course_impl		**crs_det
)
{
    ...
    UniversityO::Days days = 0;
    if (sched.get_mon_sched()) { days |= UniversityO::MONDAY;    }
    if (sched.get_tue_sched()) { days |= UniversityO::TUESDAY;   }
    if (sched.get_wed_sched()) { days |= UniversityO::WEDNESDAY; }
    if (sched.get_thu_sched()) { days |= UniversityO::THURSDAY;  }
    if (sched.get_fri_sched()) { days |= UniversityO::FRIDAY;    }

    UniversityO::ClassSchedule obv_sched;
    obv_sched.start_hour = sched.get_start_hour();
    obv_sched.duration   = sched.get_duration();
    obv_sched.class_days = days;

    *crs_det = new Course_impl(crs.get_course_number(),
			      crs.get_cost(),
			      crs.get_number_of_credits(),
			      obv_sched,
			      crs.get_number_of_seats(),
			      crs.get_title(),
			      crs.get_professor(),
			      crs.get_description());
...
UniversityO::CourseSynopsisList*
...
)
{
    ...
    try {
        ...
        UniversityO::CourseSynopsisList_var v_crs_syn_lst =
            new UniversityO::CourseSynopsisList;
...
)
{
    ...
    try {
        ...
        UniversityO::CourseSynopsisEnumerator_var v_rest = 
            UniversityO::CourseSynopsisEnumerator::_narrow(
                rest_oref.in()
            );
        ...
        if (number_remaining == 0) {
            ...
            // a "nil" one
            v_rest->destroy();
            v_rest = UniversityO::CourseSynopsisEnumerator::_nil();
...
UniversityO::Courses*
Registrar_i::get_courses(
    const UniversityO::CourseNumberList& courses
)
{
    TRACE_METHOD("Registrar_i::get_courses");
...
UniversityO::Registrar_ptr RegistrarFactory_i::find_registrar(
)
{
    ...
    try {
        ...
        return UniversityO::Registrar::_narrow(v_reg_oref.in());
    }
    HANDLE_EX
}
