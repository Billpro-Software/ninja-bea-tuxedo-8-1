...
CORBA::Boolean Server::initialize(int argc, char* argv[])
{
    ...
    s_v_fact_ref =
        TP::create_object_reference(
            _tc_SimplePerObjectFactory->id(), // factory interface id
            "SimplePerObjectFactory",        // object id
...
void Server::release()
{
    ...
    try {
        ...
        TP::userlog("Couldn't unregister the SimplePerObjectFactory");
    }
...
Tobj_Servant Server::create_servant(const char* intf_repos_id)
{
    ...
    if (!strcmp(intf_repos_id, _tc_SimplePerObjectFactory->id())) {
        return new SimplePerObjectFactory_i();
    }
