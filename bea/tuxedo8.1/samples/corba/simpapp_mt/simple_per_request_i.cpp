...
//--------------------------------------------------------------------
// Implementation of the Simple_i::_is_reentrant method. This method
// is called to determine if a servant is capable of supporting
// concurrent method invocations. We override the default 
// implementation to return True, indicating that we support 
// reentrantcy.
//--------------------------------------------------------------------

CORBA::Boolean Simple_i::_is_reentrant() 
{
    TP::userlog("_is_reentrant called in thread %lu", 
                (unsigned long)SIMPTHR_GETCURRENTTHREADID);
...
CORBA::Char* Simple_i::forward_lower(const char* val)
{
    ...
    SimplePerRequestFactory_var v_factory_ref;
    Simple_var v_simple;
    ...
    v_object = v_fact_finder_ref->find_one_factory_by_id(_tc_SimplePerRequestFactory->id());
    v_factory_ref = SimplePerRequestFactory::_narrow(v_object.in());
...
Simple_ptr SimplePerRequestFactory_i::find_simple()
{
