...
export TOBJADDR="corbalocs://${HOST}:${SPORT}"
...
*SERVERS
    ...
    ISL
        SRVGRP  = SYS_GRP
        SRVID   = 5
        CLOPT   = "-A -- -z40 -Z128 -S${SPORT} -n //${HOST}:${SPORT} -d /dev/tcp"
...
if [ "${ERROR}" = "" ]; then
    ...
    if [ "${JAVA_HOME}" != "" ]; then
        ...
        java -classpath ${CLASSPATH} -DTOBJADDR=${TOBJADDR} -Dorg.omg.CORBA.ORBpeerValidate=none SimpleClient < ${INPUT} >> ${OUTPUT} 2>&1
    fi
