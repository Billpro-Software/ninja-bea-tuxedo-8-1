...
@rem runme.cmd
@rem
@rem NT script to build and run the simple sample application
@rem
...
@if NOT defined SPORT  set SPORT=2458
@if NOT defined IPCKEY set IPCKEY=55432

@echo Testing simpapp SSL
...
@set TOBJADDR=corbalocs://%HOST%:%SPORT%
...
@rem To make simpapp scalable, browse through the ubb configuration file
@rem provided with the university production samples to help you make
@rem changes in the following sections, and use factory based routing.

@rem    a) GROUPS,     instead of a single APP_GRP create two (or more)
@rem                   application groups eg. APP_GRP1, APP_GRP2, etc.
@rem    b) SERVERS,    instead of a single simple_server specify multiple
@rem                   simple_servers each in a separate group.
@rem    c) INTERFACES, specify the interface and name the criteria to be
@rem                   used for factory based routing.
@rem    d) ROUTING,    configure how the criteria should be used to
@rem                   perform factory based routing.
...
@echo         CLOPT   = "-A -- -z40 -Z128 -S%SPORT% -n //%HOST%:%PORT%" >> %UBB% 2>> %LOG%
@echo         SEC_PRINCIPAL_NAME="IIOPListener"              >> %UBB% 2>> %LOG%
@echo         SEC_PRINCIPAL_LOCATION="IIOPListener.pem"      >> %UBB% 2>> %LOG%
@echo         SEC_PRINCIPAL_PASSVAR="ISH_PASS"      >> %UBB% 2>> %LOG%
...
@simple_client -ORBid BEA_IIOP -ORBpeerValidate none <%INPUT% >>%OUTPUT% 2>&1

@if NOT defined JAVA_HOME goto skip_run_java_client
@echo Java remote client>>%OUTPUT% 2>> %LOG%
@java -DTOBJADDR=%TOBJADDR% -Dorg.omg.CORBA.ORBpeerValidate=none -classpath %CLASSPATH% SimpleClient <%INPUT% >>%OUTPUT% 2>&1
