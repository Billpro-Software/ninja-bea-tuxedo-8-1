...
@rem runme.cmd
@rem
@rem NT script to build and run the simpapp_par sample application
@rem
...
@rem The pathname of the client output file with the pid's stripped out
@set OUTPUT_NOPID=%OUTPUT%_nopid
...
@echo     DOMAINID  simpapp_par               >> %UBB% 2>> %LOG%
...
@echo     APP_GRP3                            >> %UBB% 2>> %LOG%
@echo         LMID    = SITE1                 >> %UBB% 2>> %LOG%
@echo         GRPNO   = 4                     >> %UBB% 2>> %LOG%
...
@echo         RESTART = N                     >> %UBB% 2>> %LOG%
@echo     simple_server                       >> %UBB% 2>> %LOG%
@echo         SRVGRP  = APP_GRP3              >> %UBB% 2>> %LOG%
@echo         SRVID   = 1                     >> %UBB% 2>> %LOG%
@echo         RESTART = N                     >> %UBB% 2>> %LOG%
@echo     ISL                                 >> %UBB% 2>> %LOG%
@echo         SRVGRP  = SYS_GRP               >> %UBB% 2>> %LOG%
...
@echo native>                                   %OUTPUT%2 2>> %LOG%
@start /B cmd /C "simple_client                 <%INPUT% >>%OUTPUT% 2>&1"
@start /B cmd /C "simple_client                 <%INPUT% >>%OUTPUT%2 2>&1"
@sleep 30
@echo remote>>                            %OUTPUT% 2>> %LOG%
@echo remote>>                            %OUTPUT%2 2>> %LOG%
@start /B cmd /C "simple_client -ORBid BEA_IIOP <%INPUT% >>%OUTPUT% 2>&1"
@start /B cmd /C "simple_client -ORBid BEA_IIOP <%INPUT% >>%OUTPUT%2 2>&1"
@sleep 30

@if NOT defined JAVA_HOME goto skip_run_java_client
@echo Java remote client>>%OUTPUT% 2>> %LOG%
@echo Java remote client>>%OUTPUT%2 2>> %LOG%
@start /B cmd /C "java -DTOBJADDR=%TOBJADDR% -classpath %CLASSPATH% SimpleClient <%INPUT% >>%OUTPUT% 2>&1"
@start /B cmd /C "java -DTOBJADDR=%TOBJADDR% -classpath %CLASSPATH% SimpleClient <%INPUT% >>%OUTPUT%2 2>&1"
@sleep 30
:skip_run_java_client

@rem Get the output with pid info stripped
@findstr /V ProcessID %OUTPUT%>%OUTPUT_NOPID%
@findstr /V ProcessID %OUTPUT%>%OUTPUT_NOPID%2

@rem Get just the pid output in one file
@findstr ProcessID %OUTPUT%>%OUTPUT%_PIDS
@findstr ProcessID %OUTPUT%2>>%OUTPUT%_PIDS

@rem See if we got the expected output.
@rem Since comp asks if it should compare nore files,
@rem send in the answer "no".
@echo no | comp %OUTPUT_NOPID% %EXPECTED_OUTPUT% > NUL: 2>&1
@if ERRORLEVEL 1 goto run_error
@echo no | comp %OUTPUT_NOPID%2 %EXPECTED_OUTPUT% > NUL: 2>&1
@if ERRORLEVEL 1 goto run_error

@rem Make sure requests hit different server processes
@rem If file is same sorted forward and backward, then all 
@rem process ids were equal.  This is an error.
@sort < %OUTPUT%_PIDS > %OUTPUT%_PIDS_SORT
@sort /R < %OUTPUT%_PIDS > %OUTPUT%_PIDS_REVERSE
@echo no | comp %OUTPUT%_PIDS_SORT %OUTPUT%_PIDS_REVERSE > NUL: 2>&1
@if not ERRORLEVEL 1 goto run_error

@rem Cleanup the temporary files
@del %OUTPUT%_PIDS_SORT
@del %OUTPUT%_PIDS_REVERSE
@del %OUTPUT_NOPID%
@del %OUTPUT_NOPID%2
@del %OUTPUT%_PIDS
