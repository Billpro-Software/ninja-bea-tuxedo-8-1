...
def configureIdentityStore(propsFileLoc=None) :
    ...
    ldapHost = props.get(JpsConfigMBeanConstants.LDAP_HOST)
    ldapPort = props.get(JpsConfigMBeanConstants.LDAP_PORT)
    pwd = props.get(JpsConfigMBeanConstants.ADMIN_PASS)
    subscriberName = props.get(JpsConfigMBeanConstants.SUBSCRIBER_NAME)
    adminId = props.get(JpsConfigMBeanConstants.ADMIN_ID)
    ssl = props.get(JpsConfigMBeanConstants.SSL_LDAP)
    
    bindDN = '"'+str(adminId)+'"'
    argValue = '-autoGenerateServerId  true  -verifyRegistry   true ' 
    userName = None
    primaryAdminId = props.remove(JpsConfigMBeanConstants.PRIMARY_ADMIN_ID)
    if (primaryAdminId is not None) :
       userName = '"'+str(primaryAdminId)+'"'
       argValue = argValue + '-primaryAdminId ' + userName + ' ' 

    argValue = argValue + '-ldapServerType CUSTOM  -ldapHost ' + ldapHost + ' -ldapPort '+ ldapPort + ' '

    if (subscriberName is not None) :
        argValue = argValue + '-baseDN ' + subscriberName +' '
    
    if (bindDN is not None) :
       argValue = argValue + '-bindDN ' + bindDN + ' ' 

    if (pwd is not None ) :
        argValue = argValue + '-bindPassword ' + pwd + ' '
    
         
    userFilter = props.remove(JpsConfigMBeanConstants.USER_FILTER)
    if (userFilter is not None) :
       argValue = argValue + '-userFilter ' + userFilter + ' '

    groupFilter = props.remove(JpsConfigMBeanConstants.GROUP_FILTER)
    if(groupFilter is not None) :
       argValue = argValue + '-groupFilter ' + groupFilter + ' '
 

    userIdMap = props.remove(JpsConfigMBeanConstants.USER_ID_MAP)
    if(userIdMap is not None):
      argValue = argValue + '-userIdMap ' + userIdMap + ' ' 

    groupIdMap = props.remove(JpsConfigMBeanConstants.GROUP_ID_MAP)
    if (groupIdMap is not None) :
       argValue = argValue + '-groupIdMap ' + groupIdMap + ' '


    groupMemberIdMap = props.remove(JpsConfigMBeanConstants.GROUP_MEMBER_ID_MAP)
    if (groupMemberIdMap is not None ) :
       argValue = argValue + '-groupMemberIdMap ' + groupMemberIdMap + ' ' 
    
    if (ssl is not None) :
       argValue = argValue + '-sslEnabled ' + ssl + ' ' 

    argValue = argValue + '-ignoreCase true'
      
    argValue = '[' + argValue + ']'
    opss.configIdStoreInternal(props)
    AdminTask.configureAdminLDAPUserRegistry(argValue)
    AdminTask.setAdminActiveSecuritySettings('[-activeUserRegistry LDAPUserRegistry -enableGlobalSecurity true]')
    AdminConfig.save()
