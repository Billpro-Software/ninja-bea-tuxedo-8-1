...
def oracledmsDisplayMetricTableNames(kws):
    ...
    oracledmsVerifyParameters(kws, ("servers",))
...
def oracledmsDumpMetrics(kws):
    ...
    oracledmsVerifyConnect()
    oracledmsVerifyParameters(kws, ("servers", "format"))
    spy = oracledmsGetAggreMBean()
    ...
    dmsOraMbs = oracledmsGetOraMbs()
    try:
        dump = dmsOraMbs.invoke(spy, "dumpMetrics", 
...
def oracledmsDisplayMetricTables(names, kws):
    ...
    oracledmsVerifyConnect()
    oracledmsVerifyParameters(kws, ("servers", "variables"))
    spy = oracledmsGetAggreMBean()
    ...
    dmsOraMbs = oracledmsGetOraMbs()
    try:
        tables = dmsOraMbs.invoke(spy, "getTablesInOpenType", 
            jarray.array([nameArray, 0L, 0L, variableMap, serverArray,
                          oracledmsGetEnvNoCache()], Object),
            jarray.array(["[Ljava.lang.String;", "java.lang.Long",
    ...
    for table in tables:
        if table is None:
            ...
            handled.add(table)
            print oracledmsMetricTableToString(table)
        i = i + 1
    ora_util.hideDisplay()
    return tables
