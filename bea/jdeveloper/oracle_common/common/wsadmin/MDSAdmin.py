...
# @cancelOnException
def deleteMetadata(application, node, server, docs, restrictCustTo=None, excludeAllCust='false', excludeBaseDocs='false', excludeExtendedMetadata='false', cancelOnException='true'):
...
# @restrictCustTo Comma seperated list of customization document names or patterns 
# @remote
def exportSandboxMetadata(application, node, server, toArchive, sandboxName, restrictCustTo='%', remote='false'):
	...
	required(node,'node')
        ...
        if getBooleanFromArg(remote) == Boolean('true'):
                retValue = handler.executeMetadataTransferManagerOperation(application, node, server, operationName, params, signature, paramNames)
        else:
                retValue = executeAppRuntimeMBeanOperation(application, node, server, operationName, params, signature, paramNames)
        printMessage("OPERATION_COMPLETED_SUMMARY", operationName)
        prettyPrintValue(retValue)
...
# @sandboxName
# @remote
def importSandboxMetadata(application, node, server, fromArchive, forceSBCreation='false', useExistingSandbox='false', sandboxName=None, remote='false'):
	...
	required(node,'node')
        ...
        retValue = None
        if getBooleanFromArg(remote) == Boolean('true'):
                retValue = handler.executeMetadataTransferManagerOperation(application, node, server, transMgrOperationName, params, signature, paramNames)
        else:
                retValue = executeAppRuntimeMBeanOperation(application, node, server, operationName, params, signature, paramNames)
        printMessage("OPERATION_COMPLETED_SUMMARY", operationName)
        prettyPrintValue(retValue)
...
# @toLabel
# @remote
def exportMetadata(application, node, server, toLocation, docs='/**', restrictCustTo=None, excludeCustFor=None, excludeAllCust='false', excludeBaseDocs='false', excludeExtendedMetadata='false', excludeSeededDocs='false', fromLabel=None, toLabel=None, remote='false'):
...
# @cancelOnException
# @remote
def importMetadata(application, node, server, fromLocation, docs='/**', restrictCustTo=None, excludeAllCust='false', excludeBaseDocs='false', excludeExtendedMetadata='false', excludeUnmodifiedDocs='false', cancelOnException='true', remote='false'):
...
# @server The target server on which this application is deployed 
# @name
def createMetadataLabel(application, node, server, name):
	required(application,'application')
	required(node,'node')
	required(server,'server')
	required(name,'name')
...
# @server The target server on which this application is deployed 
# @name
def deleteMetadataLabel(application, node, server, name):
	required(application,'application')
	required(node,'node')
	required(server,'server')
	required(name,'name')
...
# @node The node on which the target server is running 
# @server The target server on which this application is deployed
def listMetadataLabels(application, node, server):
	required(application,'application')
	required(node,'node')
	required(server,'server')
...
# @server The target server on which this application is deployed
# @name
def promoteMetadataLabel(application, node, server, name):
	required(application,'application')
	required(node,'node')
	required(server,'server')
	required(name,'name')
