#!/bin/sh
##
## SSLGenCA.sh version: 11.1.1.4.0
## 
...
##
echo ""
echo "SSL Certificate Authority Generation Script: Release 11.1.1.4.0 - Production"
echo "Copyright (c) 2010 Oracle.  All rights reserved."
echo " "
export CA_ROOT=$ORACLE_HOME/rootCA
export CA_CRED=$ORACLE_HOME/credCA
export TIMESTAMP=`date +%Y%m%d%H%M%S`
export CN=CN=$TIMESTAMP-Demo-CA
export DN=$CN',O=myCompany,C=US'
...
then
   export sslDomain="cn=idm,cn=sslDomains"
else
   export sslDomain="cn=$ldapSSLdomain,cn=sslDomains"
fi
...
export existCA=`cat $LogFile|grep -i ca`
